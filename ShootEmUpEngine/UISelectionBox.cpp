...
void UISelectionBox::Update(SDL_Event *e)
{
	...
	for (int i = items.size()-1; i >= 0; i--)//start at end to ensure that the top most object will be selected
	{
		if (items[i]->isGameObject)
		{
			...
			goX = items[i]->gameObject->destRect.x;
			goY = items[i]->gameObject->destRect.y;
			goW = items[i]->gameObject->width;
			goH = items[i]->gameObject->height;
			switch (e->type)
			{
			case SDL_MOUSEBUTTONDOWN:
				if (SDL_GetMouseState(NULL, NULL) == SDL_BUTTON(SDL_BUTTON_LEFT) && mouseX > goX && mouseX < goX + goW && mouseY > goY && mouseY < goY + goH)
				{
					if (!items[i]->gameObject->isClicked)
					{
