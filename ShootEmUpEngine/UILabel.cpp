...
void UILabel::Draw()
{
	if (this)
	{
		...
		SDL_Rect textRect;
		if (alignment == 0) {
			textRect = { x, y, (drawText != nullptr) ? drawText->w : 0, drawText->h }; //input is: xpos, ypos, boxWidth, boxHeight
		}
		else {
			textRect = { (drawText != nullptr) ? (x - (drawText->w / 2)) : 0, y, (drawText != nullptr) ? drawText->w : 0, drawText->h }; //input is: xpos, ypos, boxWidth, boxHeight
		};
