...
SelectionBoxItem* UIDragDrop::HasDropped()
{
	...
	if (hasDropped)
	{
		...
		if (name == "Player")
		{
			...
			temp = new SelectionBoxItem{ "New " + name, true, new GameObject("New " + name, GameObjectType::PLAYER, mouseX, mouseY, imageDir, window, renderer) };
		}
		else if (name == "Enemy")
		{
			std::string imageDir = SDL_GetBasePath(); imageDir += "Assets\\EnemyDefault.png";
			temp = new SelectionBoxItem{ "New " + name, true, new GameObject("New " + name, GameObjectType::ENEMYWAVE, mouseX, mouseY, imageDir, window, renderer) };
		}
